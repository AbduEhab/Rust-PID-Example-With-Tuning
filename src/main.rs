...
fn main() {
    ...
    let root = BitMapBackend::new("spring_mass_damper.png", (800, 600)).into_drawing_area();
    root.fill(&WHITE).unwrap();

    let t_min = *t_values.iter().min_by(|t1, t2| t1.partial_cmp(t2).unwrap()).unwrap();
    let t_max = *t_values.iter().max_by(|t1, t2| t1.partial_cmp(t2).unwrap()).unwrap();
    let x_min = *x_values.iter().min_by(|x1, x2| x1.partial_cmp(x2).unwrap()).unwrap();
    let x_max = *x_values.iter().max_by(|x1, x2| x1.partial_cmp(x2).unwrap()).unwrap();

    let mut chart = ChartBuilder::on(&root)
        .x_label_area_size(40)
        .y_label_area_size(40)
        .margin(5)
        .caption("Spring-Mass-Damper System", ("Arial", 20))
